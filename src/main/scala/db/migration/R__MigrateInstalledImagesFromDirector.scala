...
import com.advancedtelematic.ota.deviceregistry.db.MigrateInstalledImagesFromDirector
import com.typesafe.config.ConfigFactory
import slick.jdbc.MySQLProfile.api.Database

import scala.concurrent.Future

class R__MigrateInstalledImagesFromDirector extends AppMigration with ServiceHttpClientSupport {
  ...
  override def migrate(implicit db: Database): Future[Unit] =
    new MigrateInstalledImagesFromDirector(director).run.map(_ => ())
}
